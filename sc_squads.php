...
if(mysql_num_rows($ergebnis)) {
	...
	while($db=mysql_fetch_array($ergebnis)) {
		...
		$n++;
		if(!empty($db['icon_small'])) $squadicon='<img src="images/squadicons/'.$db['icon_small'].'" style="width:338px;height:72px;margin:2px 0;" border="0" alt="'.getinput($db['name']).'" title="'.getinput($db['name']).'" />';
		else $squadicon='';
