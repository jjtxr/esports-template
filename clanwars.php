...
elseif($action=="edit") {
    ...
    for($i=1; $i<32; $i++) {
		...
		for($i=0; $i<count($map); $i++) {
			...
			while($dm=mysql_fetch_array($mapnames)) {
				$mapID=$dm['mapID'];
				if($map[$i]==$mapID) $mapname.='<option value="'.$map[$i].'" selected>&nbsp; - '.getmapname($map[$i]).'</option>';
		else $mapname.='<option value="'.$mapID.'">&nbsp; - '.$dm['map'].'</option>';
			}
			$mapname.='<option value="0" disabled="disabled">-----</option>';
      ...
      <tr>
        <td width="15%"><input type="hidden" name="map_id[]" value="'.$i.'" />map #'.($i+1).'</td>
				<td width="25%"><select size="1" name="map_name[]">'.$mapname.'</select></td>
