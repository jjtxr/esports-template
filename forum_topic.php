...
function showtopic($topic, $edit, $addreply, $quoteID, $type) {
	...
	$topicactions='<a href="printview.php?board='.$dt['boardID'].'&amp;topic='.$topic.'" target="_blank"><img src="images/icons/printview.png" border="0" alt="printview" /></a> ';
	...
	while($dr=mysql_fetch_array($replys)) {
		...
		if(!$dt['closed']) $quote='<a href="index.php?site=forum_topic&amp;addreply=true&amp;board='.$dt['boardID'].'&amp;topic='.$topic.'&amp;quoteID='.$dr['postID'].'&amp;page='.$page.'&amp;type='.$type.'"><img src="images/icons/quote.png" border="0" alt="'.$_language->module['quote'].'" /></a>';
		else $quote = "";
		...
		$actions='';
		if(($userID == $dr['poster'] OR isforumadmin($userID) OR ismoderator($userID,$dt['boardID']))&& !$dt['closed']) $actions=' <a href="index.php?site=forum_topic&amp;topic='.$topic.'&amp;edit=true&amp;id='.$dr['postID'].'&amp;page='.$page.'"><img src="images/icons/edit.png" border="0" alt="'.$_language->module['edit'].'" /></a> ';
