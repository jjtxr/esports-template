...
if($action=="show") {
	if($_GET['squadID']) {
		$getsquad = 'AND squadID="'.(int)$_GET['squadID'].'"';
	}
	else $getsquad = '';

	$ergebnis=safe_query("SELECT * FROM ".PREFIX."squads WHERE displayed='1' ".$getsquad." ORDER BY sort");
	while($ds=mysql_fetch_array($ergebnis)) {
		...
		if($ds['gamesquad']) {
			...
			eval ("\$squads_foot = \"".gettemplate("squads_foot_nongaming")."\";");
			echo $squads_foot;
		}
	}
}
	...
	if(isset($_GET['squadID'])) {
		$getsquad = 'AND squadID="'.$_GET['squadID'].'"';
	}
  ...
  while($ds=mysql_fetch_array($ergebnis)) {
    ...
    $anzmembers=mysql_num_rows(safe_query("SELECT sqmID FROM ".PREFIX."squads_members WHERE squadID='".$ds['squadID']."'"));
		...
		if($ds['icon']) $icon='<a href="index.php?site=squads&amp;action=show&amp;squadID='.$ds['squadID'].'"><img src="images/squadicons/'.$ds['icon'].'" width="338" height="72" border="0" alt="'.htmlspecialchars($ds['name']).'" /></a>';
		else $icon='';
		$inf=htmloutput($ds['info']);
