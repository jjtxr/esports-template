	...
	if(mysql_num_rows($files)) {
		...
		while($file = mysql_fetch_array($files)) {
			...
			$screens=explode("|", $ds['screens']);
				...
				$screenshots='<div><img src="images/files-pics/thumb/'.$thumb[0].'.jpg"" border="0" style="margin: 5px; padding: 2px; border: 1px solid #CECED0;" alt="" /></div>';
...
elseif(isset($_GET['file'])) {
		  ...
		  if(!empty($ds['screens'])) $screens=explode("|", $ds['screens']);
			if(is_array($screens)) {
				...
				foreach($screens as $screen) {
					if(!empty($screen)) {
                                        $thumb=explode(".", $screen);
                                        $screenshots.='<a href="images/files-pics/'.$screen.'" rel="prettyPhoto[gallery1]"><img src="images/files-pics/thumb/'.$thumb[0].'.jpg" border="0" style="margin: 5px; padding: 2px; border: 1px solid #CECED0;" alt="" /></a>';
						if($nbr==2) {
							$nbr=1;
							$screenshots.='<br />';
						} else {
	...
	if(isfileadmin($userID)) {
		...
		$admintools .= ' <input type="button" class="tab_2" onclick="MM_openBrWindow(\'upload.php?fileID='.$file['fileID'].'\',\'Files\',\'toolbar=no,status=no,scrollbars=yes,width=800,height=600\')" value="Screens" /> ';
		$admintools .= '<input type="button" onclick="MM_goToURL(\'parent\', \'index.php?site=files&amp;action=edit&amp;fileID='.$file['fileID'].'\');return document.MM_returnValue" value="'.$_language->module['edit_file'].'" class="tab_2" /> ';
		$admintools .= '<input type="button" onclick="MM_confirm(\''.$_language->module['really_delete_file'].'\', \'index.php?site=files&amp;action=delete&amp;file='.$file['fileID'].'\')" value="'.$_language->module['delete_file'].'" class="tab_2" /> ';
		$admintools .= '</td></tr>';
	}
	...
	if($file['accesslevel'] <= $accesslevel) $link = '<a href="download.php?fileID='.$fileID.'"><b><font color="#00AA00">'.str_replace('%filename%', $filename, $_language->module['download_now']).'</font></b></a>';
...
elseif($action=="report") {
	...
	if($totalcats) {
		...
		$top5 = '<table width="100%" border="0" cellspacing="1" cellpadding="3" class="tab_2">
			<tr>
				<td colspan="3" class="files_title"><b>'.$_language->module['top_5_downloads'].'</b></td>
			</tr>';
		...
		while($file = mysql_fetch_array($top5qry)) {
			...
			if($file['downloads'] != '0') {
				$top5 .= '<tr>
						<td class="border_1" width="10%" align="center"><b>'.$n.'.</b></td>
						<td class="border_1" width="70%">'.$filename.'</td>
						<td class="border_1" width="20" align="right">'.$file['downloads'].'</td>
					</tr>';
			}
			$n++;
		}
