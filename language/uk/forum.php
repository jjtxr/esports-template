<?php
/*
##########################################################################
#                                                                        #
#           Version 4       /                        /   /               #
#          -----------__---/__---__------__----__---/---/-               #
#           | /| /  /___) /   ) (_ `   /   ) /___) /   /                 #
#          _|/_|/__(___ _(___/_(__)___/___/_(___ _/___/___               #
#                       Free Content / Management System                 #
#                                   /                                    #
#                                                                        #
#                                                                        #
#   Copyright 2005-2009 by webspell.org                                  #
#                                                                        #
#   visit webSPELL.org, webspell.info to get webSPELL for free           #
#   - Script runs under the GNU GENERAL PUBLIC LICENSE                   #
#   - It's NOT allowed to remove this copyright-tag                      #
#   -- http://www.fsf.org/licensing/licenses/gpl.html                    #
#                                                                        #
#   Code based on WebSPELL Clanpackage (Michael Gruber - webspell.at),   #
#   Far Development by Development Team - webspell.org                   #
#                                                                        #
#   visit webspell.org                                                   #
#                                                                        #
##########################################################################
*/

$language_array = Array(

/* do not edit above this line */

	'add_buddy'=>'add to buddylist',
	'admin'=>'Administrator',
	'admin_actions'=>'Admin-Actions',
	'and'=>'and',
	'author'=>'Author',
	'back_buddy'=>'back to buddy-list',
	'bbcode_on'=>'<a href="javascript:MM_openBrWindow(\'code.php\',\'BBCode\',\'scrollbars=yes,width=600,height=500\')">BBCode</a> is ON',
	'board'=>'Board',
	'boards'=>'Boards',
	'clanmember'=>'Clanmember',
	'closed_folder'=>'Closed',
	'close_topic'=>'close topic',
	'closed'=>'closed',
	'closed_image'=>'<img src="images/icons/closed.gif" alt="" />',
	'default_topic_title'=>'(no title)',
	'delete_posts'=>'delete selected posts',
	'delete_topic'=>'delete topic',
	'edit'=>'edit',
	'edit_reply'=>'edit reply',
	'edit_topic'=>'edit topic',
	'forgot_message'=>'You forgot to enter a message!',
	'go'=>'GO',
	'guest'=>'1 guest',
	'guests'=>'guests',
	'happy_birthday'=>'Happy Birthday!',
	'homepage'=>'homepage',
	'html_off'=>'HTML is OFF',
	'ignore'=>'ignore user',
	'lastpost'=>'Last Post',
	'latest_post'=>'Latest Post',
	'leave_shadowtopic'=>'Leave shadowtopic?',
	'make_sticky'=>'Make this topic sticky',
	'make_topic_sticky'=>'make topic sticky',
	'make_topic_unsticky'=>'make topic unsticky',
	'mark_all_read'=>'mark all boards read',
	'mark_topics_read'=>'mark all topics read',
	'maximum_online'=>'Maximum online',
	'message'=>'Message',
	'messageboard'=>'Messageboard',
	'messenger'=>'messenger',
	'moderated_by'=>'moderated by',
	'moderator'=>'Moderator',
	'move_topic'=>'move topic',
	'move_topic_board'=>'move this topic to board',
	'moved'=>'moved',
	'n_a'=>'n/a',
	'new_posts'=>'new posts',
	'new_reply'=>'new reply',
	'new_topic'=>'new topic',
	'newest_member'=>'newest member',
	'newreply_image'=>'<img src="images/icons/newreply.gif" border="0" alt="reply" />',
	'newtopic_image'=>'<img src="images/icons/newtopic.gif" border="0" alt="new topic" />',
	'next_7_days'=>'next 7 days',
	'no_access'=>'No access',
	'no_access_write'=>'No accses to write in this board.',
	'no_icon'=>'no icon',
	'no_new_posts'=>'no new posts',
	'no_ads'=>'No advertisment avaible!',
	'no_permission'=>'You have no permission to view intern boards!',
	'not_logged'=>'Not logged in.',
	'not_logged_msg'=>'<div align="center"><div class="errorbox">To start or to reply to a topic you have to be registered and logged in!<br /><br />
    <a class="forum_err" href="index.php?site=register">register now for free!</a> - <a class="forum_err" href="index.php?site=login">login now</a></div></div>',
	'notify'=>'Notify me when a reply is posted',
	'notify_mail'=>'New reply by %poster% in topic
%topic_link%
	
%pagetitle% - %hpurl%',
	'notify_reply'=>'Notify me when a reply is posted',
	'now_online'=>'Now online',
	'options'=>'Options',
	'permission_denied'=>'Permission denied!',
	'popular'=>'popular',
	'post_new_reply'=>'post new reply',
	'post_new_topic'=>'post new topic',
	'posts'=>'Posts',
	'posts_in'=>'posts in',
	'preview'=>'Preview',
	'preview_post'=>'preview my post',
	'quote'=>'quote',
	'read_only'=>'read only',
	'registered'=>'registered',
	'registered_user'=>'1 registered user',
	'registered_users'=>'registered users',
	'reopen_topic'=>'reopen topic',
	'replies'=>'Replies',
	'search_image'=>'<img src="images/icons/search.gif" border="0" alt="" />',
	'select_all'=>'select all',
	'smilies_on'=>'<a href="javascript:MM_openBrWindow(\'smileys.php\',\'Smileys\',\'scrollbars=yes,width=340,height=500\')">Smilies</a> are ON',
	'sort'=>'Sort',
	'stats'=>'Stats',
	'sticky'=>'sticky',
	'subject'=>'subject',
	'today'=>'today',
	'topic'=>'Topic',
	'topic_closed'=>'The topic has been closed!',
	'topic_icon'=>'topic-icon',
	'topic_not_found'=>'Topic could not be found.',
	'topics'=>'Topics',
	'views'=>'Views',
	'who_was_online'=>'who was online',
	'whos_online'=>'Who\'s online?',
	'wrote'=>'wrote',
	'yesterday'=>'yesterday',
	'enter_topic'=>'You have to enter a topic title!',
	'enter_text'=>'You have to enter some text!'
);
?>