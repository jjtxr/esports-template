...
if($guests==1) $guests_on='1 '.$_language->module['guest'];
	...
	$news=mysql_num_rows(safe_query("SELECT newsID FROM ".PREFIX."news WHERE published='1'"));
	$articles=mysql_num_rows(safe_query("SELECT articlesID FROM ".PREFIX."articles"));
	$cw=mysql_num_rows(safe_query("SELECT cwID FROM ".PREFIX."clanwars"));	
	$awards=mysql_num_rows(safe_query("SELECT awardID FROM ".PREFIX."awards"));


	$squads= mysql_num_rows(safe_query("SELECT squadID FROM ".PREFIX."squads"));
	$squadsm= mysql_num_rows(safe_query("SELECT userID FROM ".PREFIX."squads_members"));
