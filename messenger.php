...
elseif(isset($_POST['quickaction'])) {
	...
	if(isset($_POST['messageID'])){
		...
		if($quickactiontype=="viewed") {
			foreach($messageID as $id) {
				safe_query("UPDATE ".PREFIX."messenger SET viewed='1' WHERE messageID='$id' AND touser='$userID'"); # PM Mod 
			}
		}
		elseif($quickactiontype=="notviewed") {
			foreach($messageID as $id) {
				safe_query("UPDATE ".PREFIX."messenger SET viewed='0' WHERE messageID='$id' AND touser='$userID'"); # PM Mod 
			}
		}
...
elseif($userID) {
    ...
    $anz=mysql_num_rows($ergebnis);
      ...
      $n=1;
		...
		else echo '<tr><td colspan="5">'.$_language->module['no_incoming'].'</td></tr>';
    ...
    echo $pm_outgoing_head;
      ...
      $n=1;
			while($ds=mysql_fetch_array($ergebnis)) {
				...
				if(!$ds['viewed']) { $icon='<img src="images/icons/pm_old.gif" width="14" height="12" alt="" />'; } #PM Mod
				else {$icon='<img src="images/icons/pm_new.gif" width="14" height="12" alt="" />'; } # PM Mod
		...
		else echo '<tr><td colspan="4">'.$_language->module['no_outgoing'].'</td></tr>';
