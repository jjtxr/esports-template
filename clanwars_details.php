...
if(!isset($_GET['action'])) {
  ...
  if(!empty($ds['screens'])) $screens=explode("|", $ds['screens']);
	if(is_array($screens)) {
		...
		foreach($screens as $screen) {
			if(!empty($screen)) {
				$screenshots.='<a href="images/clanwar-screens/'.$screen.'" target="_blank">#'.$n.' Screenshot</a>';
				if($nbr==2) {
					$nbr=1;
					$screenshots.='<br />';
				} else {
  ...
  if(!(mb_strlen(trim(isset($screenshots))))) $screenshots=$_language->module['no_screenshots'];
